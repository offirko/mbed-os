...
int bootloader_get_device_key(BlockDevice *bd, uint32_t tdb_start_offset, uint32_t tdb_end_offset, void *data_buf, size_t data_buf_size, size_t *actual_data_size)
{
	...
	if (tdb_end_offset > bd->size()) {
		tr_error();
	}

	calc_area_params(bd, tdb_start_offset, tdb_end_offset, area_params);

	for (uint8_t active_area = 0, active_area < TDBSTORE_NUMBER_OF_AREAS, active_area++) {

		status = reserved_data_get(area_params, active_area, data_buf, data_buf_size, actual_data_size);
...
static void calc_area_params(BlockDevice *bd, uint32_t tdb_start_offset, uint32_t tdb_end_offset, tdbstore_area_data_t *area_params)
{
    // Entire TDBStore can't exceed 32 bits
    bd_size_t bd_size = (bd_size_t)(tdb_end_offset - tdb_start_offset);

    size_t area_0_size = 0;
    bd_size_t prev_erase_unit_size = bd->get_erase_size(area_0_size);
    bool _variant_bd_erase_unit_size = 0;
    ...
    area_params[0].size = area_0_size;
    area_params[1].address = area_0_size;
    area_params[1].size = bd_size - area_0_size;
}

static int reserved_data_get(BlockDevice *bd, tdbstore_area_data_t *area_params, uint8_t active_area, void *reserved_data, size_t reserved_data_buf_size, size_t *actual_data_size)
{
    ...
    ret = read_area(active_area, RESERVED_AREA_SIZE, sizeof(trailer), &trailer);
    if (ret) {
        return ret;
    }
    ...
    while (actual_size) {
        uint32_t chunk = (uint32_t) actual_size;
        ret = read_area(bd, area_params, active_area, offset, chunk, buf);
        if (ret) {
            return ret;
        }
...
int TDBStore::read_area(tdbstore_area_data_t *area_params, uint8_t active_area, uint32_t offset, uint32_t size, void *buf)
{
    int os_ret = _buff_bd->read(buf, _area_params[area].address + offset, size);
